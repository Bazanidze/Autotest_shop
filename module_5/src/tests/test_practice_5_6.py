...
class TestExample:
    def test_1(self, set_up_browser):
        ...
        # Ниже проверяю каждый элемент на странице (я пробовал использовать text_to_be_present_in_element (закоментировано ) можно было с ним как то сделать ? и вообще правильно его использовал??? И для чего он? Я как понял он должен был брать текст из атрибута)
