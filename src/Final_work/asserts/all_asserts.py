...
def check_summ_products_basket(selenium):
    with allure.step('Проверка: Итоговая сумма товаров в корзине отображается корректно'):
        ...
        summ_products = selenium.find_element(By.CSS_SELECTOR, 'td[data-title="Сумма"]').text
...
def check_summ_after_payment(selenium, summ_products):
    with allure.step('Проверка: После оформления заказа, сумма товаров указана корректно'):
        end_total_summ = selenium.find_element(By.XPATH, '//th[contains(text(), "Total")]/following-sibling::td').text
