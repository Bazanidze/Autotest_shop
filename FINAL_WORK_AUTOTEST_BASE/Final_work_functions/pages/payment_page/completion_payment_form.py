...
def completion_payment_form(selenium):
    ...
    with (allure.step('Заполнение страницы оформления заказа:')):
        ...
        with allure.step('Выбор даты заказа на завтра'):
            ...
            action_chains.move_to_element(button_calendar).move_by_offset(xoffset=250, yoffset=0).click(
            ).send_keys(Keys.ARROW_RIGHT + Keys.ENTER).perform()
            action_chains.release()

        with allure.step('Выбор способа оплаты: "Оплата при доставке"'):
            selenium.find_element(By.XPATH, '(//input[@class="input-radio"])[2]').click()

        with allure.step('Выбор чекбокса: "Ознакомился с условиями использования веб-сайта и согласен с ними"'):
        ...
        entered_data = first_name + ' ' + last_name + '\n' + street_house + '\n' + city + '\n'\
            + state + '\n' + postcode + '\n' + number_phone + '\n' + email_address
