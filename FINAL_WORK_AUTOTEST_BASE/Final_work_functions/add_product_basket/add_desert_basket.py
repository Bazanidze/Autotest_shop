...
def add_dessert_basket(selenium):
    with allure.step("Выбор десерта"):
        time.sleep(0.5)
        name_dessert = selenium.find_element(
            By.XPATH, '(//div[@class="wc-products"]//h3)[1]'
        ).text
        price_dessert = selenium.find_element(
            By.XPATH, '(//div[@class="price-cart"]//span[@class="price"])[1]'
        ).text
        price_dessert = normal_price_product(price_dessert)

        with allure.step(
            f"Добавление десерта: '{name_dessert}' стоимостью {price_dessert} в корзину"
        ):
